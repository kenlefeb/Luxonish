...
namespace Tests.DateTime
{
    ...
    public class CreateLocal
    {
        ...
        [Theory, AutoData]
        public void WithYear_ReturnsBeginningOfThatYear(int year)
        {
            // act
            var actual = Luxonish.DateTime.CreateLocal(year);

            // assert
            actual.Year.Should().Be(year, because: $"DateTime.CreateLocal({year}) should return the beginning of the year specified");
            actual.Month.Should().Be(1);
            actual.Day.Should().Be(1);
            actual.Hour.Should().Be(0);
            actual.Minute.Should().Be(0);
